...
// !CRF! - modified by Claudio Fahey to allow for multiple To: and Cc:
// recipients.
//////////////////////////////////////////////////////////////////////
...
#else
  for (n = 0;n < Recipient.GetSize();n++)
  {
    if (n > 0)
		bstrValue += _T(", ");
    if (Recipient[n].Name.Length())
    {
      wsprintf(szOut,_T("\"%s\" "),OLE2T(Recipient[n].Name));
      bstrValue += szOut;
    }
    wsprintf(szOut,_T("<%s>"),OLE2T(Recipient[n].Address));
    bstrValue += szOut;
  }
  Headers.Add(bstrHeader,bstrValue);
#endif
...
/* !CRF! - same as wcslen but stops after length reaches maxlen */
static size_t __cdecl wcsminlen(const wchar_t *wcs, size_t maxlen)
	{
	size_t len = 0;
	while (maxlen-- && *(wcs++))
		len++;
...
BSTR CSmtpMessage::BreakMessage(BSTR bstrSrc, int nLength)
{
  ...
  unsigned nTok1Len = 0;
  LPOLESTR pszMsg = 0;
  unsigned nMsgLen = 0, nMsgMaxLen = 0, nNewMsgLen = 0;

  if (!bstrSrc) return NULL;

  printf("CSmtpMessage::BreakMessage: begin\n");

  // Process each line one at a time
  pszTok1 = wcstok(bstrTemp.m_str,OLESTR("\r\n"));
  while (pszTok1)
  {
	// initial size of dest buffer assumes extra CR+LF for every 76 characters
    nMsgLen = 0;
	nMsgMaxLen = wcslen(pszTok1) * (76 + 2) / 76;
	pszMsg = (LPOLESTR) malloc((nMsgMaxLen+1) * sizeof(OLECHAR));
	printf("CSmtpMessage::BreakMessage: initial nMsgMaxLen=%u\n", nMsgMaxLen);

	for (;;)
    {
	  //printf("CSmtpMessage::BreakMessage: begin wcslen(pszTok1)\n");
      //nTok1Len = wcslen(pszTok1);
	  nTok1Len = wcsminlen(pszTok1, 77);
	  //printf("CSmtpMessage::BreakMessage: wcslen(pszTok1)=%d\n", nTok1Len);
      if (nTok1Len <= 76)
		  break;
	  ...
	  //printf("CSmtpMessage::BreakMessage: (1) pszTok2=%p\n", pszTok2);
#if 0	// !CRF! - performance fix
      if (wcspbrk(pszTok1,pszBreaks))		// !CRF! - too slow because this searches entire attachment
      {
        while (!wcschr(pszBreaks,*pszTok2) && pszTok2 > pszTok1)
          pszTok2--;
      }
#else
	  while (!wcschr(pszBreaks,*pszTok2))
		  {
          pszTok2--;
		  if (pszTok2 < pszTok1)
			  {
			  pszTok2 = pszTok1 + 75;		// break not found; force break at 76th character
			  break;
			  }
		  }
#endif
	  //printf("CSmtpMessage::BreakMessage: (2) pszTok2=%p\n", pszTok2);
      // Add the first part of the message to the final output, add a CRLF
	  nNewMsgLen = nMsgLen + (pszTok2 - pszTok1 + 1) + 2;
	  if (nNewMsgLen > nMsgMaxLen)
		  {
		  nMsgMaxLen = 4096 * ((nNewMsgLen / 4096) + 1) - 1;
		  pszMsg = (LPOLESTR) realloc(pszMsg, (nMsgMaxLen+1) * sizeof(OLECHAR));
		  printf("CSmtpMessage::BreakMessage: realloc nMsgMaxLen=%u\n", nMsgMaxLen);
		  }
      //bstrMsg.Append(pszTok1,(pszTok2 - pszTok1) + 1);
	  memcpy(pszMsg + nMsgLen, pszTok1, (pszTok2 - pszTok1 + 1) * sizeof(OLECHAR));
	  nMsgLen += pszTok2 - pszTok1 + 1;

      //bstrMsg += _T("\r\n");
	  memcpy(pszMsg + nMsgLen, OLESTR("\r\n"), 3 * sizeof(OLECHAR));
	  nMsgLen += 2;

	  //printf("CSmtpMessage::BreakMessage: (5) nMsgLen=%u, "
	  //	"wcslen(pszMsg)=%d\n", nMsgLen, wcslen(pszMsg));

	  //wprintf(OLE("CSmtpMessage::BreakMessage: (6) pszMsg=[%s]\n"), pszMsg);

      // Replace the original 77th character back, and advance to that character.
      pszTok1 = &pszTok2[1];
    }
	printf("CSmtpMessage::BreakMessage: (3) nMsgLen=%u, bstrMsg.Length=%u, "
		"wcslen(pszMsg)=%d\n", nMsgLen, bstrMsg.Length(), wcslen(pszMsg));

	bstrMsg.Append(pszMsg, nMsgLen);
	printf("CSmtpMessage::BreakMessage: (4) bstrMsg.Length=%u\n", bstrMsg.Length());

	free(pszMsg);
	pszMsg = 0; nMsgLen = 0; nMsgMaxLen = 0;

    bstrMsg += pszTok1;
    bstrMsg += _T("\r\n");
    pszTok1 = wcstok(NULL,OLESTR("\r\n"));
  }
  printf("CSmtpMessage::BreakMessage: end\n");
  return bstrMsg.Detach();
}
...
int CSmtp::SendMessage(CSmtpMessage &msg)
{
  ...
  //if (!msg.Recipient.Address.Length() && !msg.CC.GetSize()) return -1;		// !CRF!
  if (!msg.Recipient.GetSize()) return -1;
...
#else
  for (n = 0;n < msg.Recipient.GetSize();n++)
  {
    nRet = SendTo(OLE2T(msg.Recipient[n].Address));
  }
#endif
...
int CSmtp::SendData(CSmtpMessage &msg)
{
  ...
  LPSTR pszCmd = ATLW2AHELPER(lpa, _lpw, _convert);

  //LPSTR pszCmd = OLE2T(bstrMsg);	// !CRF! - fix stack overflow
  nRet = SendCmd(pszCmd);
  free(lpa);
  if (nRet != 250) RaiseError(nRet);
