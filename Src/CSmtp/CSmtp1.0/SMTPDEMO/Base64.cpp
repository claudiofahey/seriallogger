...
LPCTSTR CBase64::DecodedMessage() const 
{ 
  return (LPCTSTR) m_pDBuffer;
}

LPCTSTR CBase64::EncodedMessage() const
{ 
...
{
  if(szMessage != NULL)
    CBase64::Encode((const PBYTE)szMessage, lstrlen(szMessage));
}
...
{
  if(szMessage != NULL)
    CBase64::Decode((const PBYTE)szMessage, lstrlen(szMessage));
}
