...
/*
 ...
 !CRF! - modified by Claudio Fahey (claudio@muconsulting.com) 4/11/02
...
#else
  // Write out all the To names
  for (n = 0;n < Recipients.GetSize();n++)
  {
    if (strValue.length()) strValue += _T(",\r\n\t");
    if (Recipients[n].Name.length())
    {
      wsprintf(szOut,_T("\"%s\" "),Recipients[n].Name.c_str());
      strValue += szOut;
    }
    wsprintf(szOut,_T("<%s>"),Recipients[n].Address.c_str());
    strValue += szOut;
  }
  Headers.Remove(strHeader);
  Headers.Add(strHeader,strValue);

  // Write out the CC header
  if (CC.GetSize())
  {
	strValue.erase();
	strHeader = _T("Cc");
...
#else
    wsprintf(szOut,_T("--=_CSmtpMsgPart123X456_001_%8.8X"),GetTickCount());
#endif
    strBoundary = szOut;
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
      ...
      if (ReadFile(hFile,pData,dwSize,&dwBytes,NULL))
      {
        ...
        if (!Attachments[n].AltName.length())
        {
          ...
          pszFile = _tcsrchr(szFilename,'\\');	  // !CRF! - TODO: also check for / or :
...
// !CRF! - TODO: has very poor performance with large text files (3 MB)
// !CRF! - BUG!: Does not handle text files with embedded 0x00 characters. The
// work around is to force Base64 encoding always.
void CSmtpMessage::EncodeQuotedPrintable(String& strDest, String& strSrc)
{
...
#if 1	// !CRF! - default allocator is very slow because it grows space in small chunks
  // allocate enough for entire string; allow for =CRLF after every 75 characters.
  strTemp.reserve(strSrc.length() * (75+3)/75);
  strDest.reserve(strSrc.length() * (75+3)/75);
#endif
  ...
  do
  {
    ...
    {
	  //printf("EncodeQuotedPrintable: strTemp.length()=%d\n", strTemp.length());
      ch = *pszTok2;
      *pszTok2 = 0;
      strTemp += pszTok1;
      *pszTok2 = ch;
...
void CSmtpMessage::EncodeMessage(EncodingEnum code, String& strMsg, String& strMethod, LPBYTE pByte, DWORD dwSize)
{
  ...
  if (!pByte)
  {
#ifndef _UNICODE	// !CRF! - UNICODE only fix; recommended by http://www.codeguru.com/mfc/comments/25420.shtml
  pszBuffer = (LPSTR)malloc(strMsg.length() + 1);
    _T2A(pszBuffer,strMsg.c_str());
#else
  int iLenOfWideCharStr = WideCharToMultiByte(CP_ACP,0,strMsg.c_str(),-1,NULL,0,NULL,NULL); 
  pszBuffer = (LPSTR)malloc(iLenOfWideCharStr * sizeof(char)); 
  WideCharToMultiByte(CP_ACP,0,strMsg.c_str(),-1,pszBuffer,iLenOfWideCharStr,NULL,NULL);
#endif
...
#if 1	// !CRF! - bug fix recommended by http://www.codeguru.com/mfc/comments/34647.shtml
  if (dwLen == 0)
	{
#ifdef _DEBUG
	TCHAR szOut[MAX_PATH];
...
#if 0	// !CRF! - recommended by http://www.codeguru.com/mfc/comments/34810.shtml
...
int CSmtp::SendCmd(LPTSTR pszCmd)
{
...
#if 0	// !CRF! - bug fix from http://www.codeguru.com/mfc/comments/24079.shtml
  FD_ZERO(&set);
#endif
  ...
  if (pszCmd)
  {
    ...
    while (nRet > 0)
    {
#if 1	// !CRF! - bug fix from http://www.codeguru.com/mfc/comments/24079.shtml
	  FD_ZERO(&set);
#endif
...
#if 0	// !CRF! - bug fix from http://www.codeguru.com/mfc/comments/25464.shtml
      if (nRet == 1) nRet = recv(m_hSocket,szResult,CMD_RESPONSE_SIZE,0);
#else
      if (nRet > 0) nRet = recv(m_hSocket,szResult,CMD_RESPONSE_SIZE,0);
#endif
    }
    dwPosition = 0;
...
void CSmtp::SmtpCommandResponse(LPTSTR pszCmd, int /*nResponse*/, LPTSTR pszResponse)
{
  ...
  if (pszCmd)
  {
#if 0	// !CRF! - takes too long for large commands; show first 4 KB instead
...
#else
    TCHAR szOut[DEBUG_COMMAND_MAX];
    OutputDebugString(_T("SmtpCommand : "));
    lstrcpyn(szOut,pszCmd,DEBUG_COMMAND_MAX);
    OutputDebugString(szOut);
#endif
...
#else
  if (!msg.Recipients.GetSize() && !msg.CC.GetSize()) return -1;
#endif
...
#else
  for (n = 0;n < msg.Recipients.GetSize();n++)
  {
    nRet = SendTo((LPTSTR)msg.Recipients[n].Address.c_str());
    if (!nRet) nRecipientCount ++;
  }
#endif
